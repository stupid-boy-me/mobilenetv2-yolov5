...
def nms(boxes, thresh, mode='inter'):
    ...
    while len(sort_boxes) > 0:
        ...
        if len(sort_boxes) > 1:
            _boxes = sort_boxes[1:]
            # _boxes=sort[sort_boxes[:,-1]==_box[-1]]
...
#             print(_boxes.shape)
#             __cls=_boxes[...,6]
#             _cls_boxes=_boxes[_cls ==__cls]
#             not_cls_boxes=[_cls!=__cls]
#             # _boxes=sort[sort_boxes[:,-1]==_box[-1]]
#             _iou = iou(_box, _cls_boxes, mode)
#             sort_boxes1 = _boxes[_iou < thresh]
#             print(sort_boxes1.shape,not_cls_boxes.shape)
#             sort_boxes=torch.cat(sort_boxes1,not_cls_boxes,dim=1)
#         else:
#             break
#
#     return keep_boxes
